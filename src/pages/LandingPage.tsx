        ...
        <Stack divider={<Divider flexItem />}>
          ...
          >
            <Box textAlign="center" mt={2}>
              <Link to="/assessment-metrics" style={{ textDecoration: "none" }}>
...
const FeatureStack: React.FC<FeatureStackProps> = ({ image, title, desc, children }) => {
    return (
      <Stack direction={{ sm: "column", md: "row" }} spacing={2} margin={5}>
        <Box flex={1}>{image}</Box>
        <Stack sx={{ flex: 2 }}>
          <Typography fontSize={"2.8rem"} textAlign={"center"}>
            {title}
          </Typography>
