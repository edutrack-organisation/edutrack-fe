import { Box, CircularProgress, Typography, Stack, Button, Divider } from "@mui/material";
...
const ViewPdfPage = () => {
    ...
    const [isFetchingTitleList, setIsFetchingTitleList] = useState<Boolean>(true);
    ...
    useEffect(() => {
        Api.getPdfTitleList().then((response) => {
            ...
            if (title) { // If there is a paper that requires to be automatically redirected
        ...
        return (
            <Stack flex={1} divider={<Divider flexItem />} >
                {titleList.map((title, index) => 
                    <Button 
                        sx={{ justifyContent: "flex-start", height: "5rem", flexShrink: 0 }}
    ...
    return (
        ...
        >
            ...
            >
                {isFetchingTitleList ? (
                    <Box sx={{ flex: 1, alignSelf: "center" }}>
                        <CircularProgress size={"100px"} sx={{ color: theme.colors.highlight1, alignSelf: "center" }} />
                    </Box>
                ) : (
                    <SelectPaperPanel />
                )}
            ...
            {/* Vertical Divider */}
            <Divider orientation="vertical" flexItem sx={{ backgroundColor: theme.colors.highlight1, width: "0.25rem", borderRadius: 3, margin: 1 }} />
